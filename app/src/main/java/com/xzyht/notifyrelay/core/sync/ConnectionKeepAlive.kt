...
class ConnectionKeepAlive(
    ...
    fun startHeartbeatToDevice(uuid: String, initialIp: String, initialPort: Int, sharedSecret: String) {
        ...
        if (BuildConfig.DEBUG) Log.d("死神-NotifyRelay", "[KeepAlive] startHeartbeatToDevice: uuid=$uuid, ip=$initialIp, port=$initialPort")
        ...
        val job = scope.launch {
            ...
            while (true) {
                ...
                try {
                    ...
                    if (BuildConfig.DEBUG) Log.d("死神-NotifyRelay", "[KeepAlive] 心跳发送失败: $uuid, ${e.message}")
                }
    ...
    fun handleHeartbeatFailure(uuid: String) {
        if (BuildConfig.DEBUG) Log.w("死神-NotifyRelay", "[KeepAlive] 心跳连续失败5次，自动停止心跳并尝试重连: $uuid")
        heartbeatJobs[uuid]?.cancel()
        ...
        scope.launch {
            ...
            if (!ip.isNullOrEmpty() && ip != "0.0.0.0") {
                for (attempt in 1..3) {
                    if (BuildConfig.DEBUG) Log.d("死神-NotifyRelay", "[KeepAlive] 心跳失败后重连尝试 $attempt/3: $uuid, $ip:$port")
                    deviceManager.connectToDevice(DeviceInfo(uuid, displayName, ip, port))
                    delay(2000)
                    if (heartbeatedDevices.contains(uuid)) {
                        if (BuildConfig.DEBUG) Log.d("死神-NotifyRelay", "[KeepAlive] 心跳失败后重连成功: $uuid")
                        return@launch
                    }
                }
                if (BuildConfig.DEBUG) Log.w("死神-NotifyRelay", "[KeepAlive] 心跳失败后重连失败，设备离线: $uuid")
            }
        }
        ...
        scope.launch {
            while (true) {
                ...
                if (deviceManager.isWifiDirectNetworkInternal()) {
                    ...
                    if (BuildConfig.DEBUG) Log.d("死神-NotifyRelay", "[KeepAlive] WLAN直连定期检查：${authed.size}个认证设备")
                    ...
                    for ((deviceUuid, auth) in authed) {
                        ...
                        if (!isOnline) {
                            ...
                            val port = info?.port ?: auth.lastPort ?: deviceManager.listenPort
                            if (!ip.isNullOrEmpty() && ip != "0.0.0.0") {
                                if (BuildConfig.DEBUG) Log.d("死神-NotifyRelay", "[KeepAlive] WLAN直连定期重连离线设备: $deviceUuid, $ip:$port")
    ...
    suspend fun performDeviceConnectionWithRetry(device: DeviceInfo, maxRetries: Int): Pair<Boolean, String?> {
        ...
        for (retry in 0 until maxRetries) {
            try {
                ...
                if (BuildConfig.DEBUG) Log.d("死神-NotifyRelay", "connectToDevice: handshake resp=$resp")
                ...
                if (resp != null && resp.startsWith("ACCEPT:")) {
                    ...
                    if (parts.size >= 3) {
                        ...
                        if (BuildConfig.DEBUG) Log.d("死神-NotifyRelay", "认证成功，启动心跳: uuid=${device.uuid}, ip=${device.ip}, port=${device.port}")
                        ...
                        if (device.uuid != deviceManager.uuid) {
                            ...
                            if (myInfo != null) {
                                if (!heartbeatedDevices.contains(device.uuid)) {
                                    if (BuildConfig.DEBUG) Log.d("死神-NotifyRelay", "认证成功后自动反向connectToDevice: myInfo=$myInfo, peer=${device.uuid}")
                                    deviceManager.connectToDevice(myInfo)
                                } else {
                                    if (BuildConfig.DEBUG) Log.d("死神-NotifyRelay", "对方已建立心跳，不再反向connectToDevice: peer=${device.uuid}")
                                }
                            } else {
                                if (BuildConfig.DEBUG) Log.d("死神-NotifyRelay", "本机getDeviceInfo返回null，无法反向connectToDevice")
                            }
                        }
                        return Pair(true, null)
                    } else {
                        if (BuildConfig.DEBUG) Log.d("死神-NotifyRelay", "认证响应格式错误: $resp")
                        return Pair(false, "认证响应格式错误")
                    }
                } else if (resp != null && resp.startsWith("REJECT:")) {
                    if (BuildConfig.DEBUG) Log.d("死神-NotifyRelay", "对方拒绝连接: uuid=${device.uuid}")
                    return Pair(false, "对方拒绝连接")
                } else {
                    if (BuildConfig.DEBUG) Log.d("死神-NotifyRelay", "认证失败: resp=$resp")
        ...
        if (BuildConfig.DEBUG) Log.e("死神-NotifyRelay", "connectToDevice所有重试失败: ${lastException?.message}")
